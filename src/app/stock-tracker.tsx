...
export default function StocksTracker() {
  ...
  const calculateDaysRemaining = (purchaseDate: string, days: number) => {
    ...
    const now = new Date()
    const remaining = Math.ceil((target.getTime() - now.getTime()) / (1000 * 60 * 60 * 24))
  ...
  const createNewStock = () => {
    if (!currentStock.stockName || !currentStock.purchasePrice || !currentStock.purchaseDate) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 p-4">
      <div className="container mx-auto">
        ...
        <p className="text-center mb-6 text-gray-600 dark:text-gray-400">{currentDateTime}</p>
              ...
              <div className="space-y-2">
                ...
                {currentStock.purchasePrice && (
                  <div className="text-sm text-muted-foreground mt-2">
                    <p>20% Profit Target: ${calculateProfitTargets(currentStock.purchasePrice).target20}</p>
                    <p>30% Profit Target: ${calculateProfitTargets(currentStock.purchasePrice).target30}</p>
                    <p>50% Profit Target: ${calculateProfitTargets(currentStock.purchasePrice).target50}</p>
                    <p>70% Profit Target: ${calculateProfitTargets(currentStock.purchasePrice).target70}</p>
                    <p>80% Profit Target: ${calculateProfitTargets(currentStock.purchasePrice).target80}</p>
                    <p>100% Profit Target: ${calculateProfitTargets(currentStock.purchasePrice).target100}</p>
                  </div>
                )}
              </div>
              <div className="space-y-2">
            ...
            return (
              <Card key={stock.id} className="bg-white dark:bg-gray-800">
                <CardHeader>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">{stock.timestamp}</p>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2">
                    <p>
                    ...
                    <p>
                      <strong>Purchase Date:</strong> {new Date(stock.purchaseDate).toLocaleDateString()}
                    </p>
                    ...
                    {days90 <= 5 && days90 > 0 && (
                      <Alert>
                        ...
                        <AlertDescription>Only {days90} days remaining until 90-day mark</AlertDescription>
                      </Alert>
                    )}
                    ...
                    {days180 <= 5 && days180 > 0 && (
                      <Alert>
                        ...
                        <AlertDescription>Only {days180} days remaining until 180-day mark</AlertDescription>
                      </Alert>
                    )}
